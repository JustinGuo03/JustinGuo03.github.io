...
export class WordleSolverComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.dfWords.forEach((word:DataFrame) => {
          const letter = word[`letter_${i + 1}`];
          counter.update([letter]);
      });
      this.letterFrequency.push(counter);
    }

      // Calculate word frequency for each word
    const word_frequency: { [key: string]: number } = {};
    for (const word of this.listPossibleAnswers) {
      word_frequency[word] = this.findWordFrequency(word);
    }

    // Add wordFreq to df_words
    this.dfWords.forEach((wordData:DataFrame) => {
      const word: Word = {
        id: null,
        letter1: wordData[`letter_1`],
        letter2: wordData[`letter_2`],
        letter3: wordData[`letter_3`],
        letter4: wordData[`letter_4`],
        letter5: wordData[`letter_5`],
        score: this.findWordFrequency(wordData['word']),
        word: wordData['word']
      };
    ...
    // Sort df_words by wordFreq in descending order
    this.allWords.sort((a, b) => (b.score || 0) - (a.score || 0));
  ...
  findWordFrequency(word: string): number {
    let total = 0;
    for (let i = 0; i < word.length; i++) {
        const letter = word[i].toUpperCase();
        total += this.letterFrequency[i].get(letter);
    }
    return total;
  }
  ...
  changeColor(index: number) {
    this.colorIndices[index] = (this.colorIndices[index] + 1) % this.colors.length;
  }
